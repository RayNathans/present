...
<head>
    ...
    <title>Edit Bahan Baku Item</title>
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto p-6">
        <header class="mb-4">
            <h1 class="text-3xl font-bold">Edit Bahan Baku Item</h1>
        </header>

        <form action="{{ route('bahanbaku.update', $bahanbaku->id) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="space-y-12">
                <div class="border-b border-gray-900/10 pb-12">
                    ...
                    <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                        <div class="sm:col-span-4">
                            ...
                            <div class="mt-2">
                                <input type="text" name="name" id="name" value="{{ $bahanbaku->nama_bahan_baku }}" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm" placeholder="Enter menu name" required>
                            </div>
                            @error('name')
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <div class="mt-2">
                                <input type="number" name="price" id="price" value="{{ $bahanbaku->harga_bahan_baku }}" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm" placeholder="Enter price" required>
                            </div>
                            @error('price')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="mt-6 flex items-center justify-end gap-x-6">
                    ...
                    <button type="submit" class="w-full rounded-lg bg-gray-900 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-gray-700">Perbarui</button>
                </div>
            </div>
        </form>
    </div>
